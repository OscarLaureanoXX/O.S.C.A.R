...
lectura	    :	'read' '(' ')' ;
expresion	:	exp ((RELACIONALES | LOGICOS){#meter operador} exp)? ;
exp		    :	termino (( '+' | '-' ){#meter operador} termino)* ;
termino	    :	factor (( '*' | '/' | '%' ) {#meter operador} factor)* ;
factor	    :	('(' {#meter parentesis} expresion ')'{#sacar parentesis}) | (( '+' | '-')? var_cte) | llamadaret;
